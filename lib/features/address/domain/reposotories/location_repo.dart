...
class LocationRepo {
  ...
  Future<ApiResponseModel> getAddressFromGeocode(LatLng latLng) async {
    try {
      final dio = Dio(); // Nouveau Dio sans interceptor

      final String url =
          '${AppConstants.geocodeUri}?latlng=${latLng.latitude},${latLng.longitude}&key=${AppConstants.googleMapApiKey}';
  ...
  try {
    ...
    Response response = await dioClient!.get(
        ...
        options: Options(
          headers: {
            // Explicitly override to ensure Authorization is not included
            'Authorization': null,
        },
      ),
    );
