...
class LocationProvider with ChangeNotifier {
  ...
  void updatePosition(
      CameraPosition? position, bool fromAddress, String? address,
    ...
    if (_updateAddAddressData) {
      ...
      try {
        if (fromAddress) {
          print(
              '---------------(FROM ADDRESS POSITION)---------${position?.target.latitude} and ${position?.target.longitude}');
          _position = Position(
          ...
          print(
              '---------------(FROM ADDRESS Pick Positon)---------${position?.target.latitude} and ${position?.target.longitude}');
        ...
        if (_changeAddress) {
          ...
          print(
              "------(CHANGE ADDRESS)----------Address: $address and PickedAddress: $_pickAddress and Force Notify : $forceNotify");
          notifyListeners();
        } else {
          _changeAddress = true;
        }
