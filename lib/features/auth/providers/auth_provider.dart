...
class AuthProvider with ChangeNotifier {
  ...
  final GoogleSignIn _googleSignIn = kIsWeb
      ? GoogleSignIn(
    clientId: "227254692534-1ncs6vu10llev6engeqt22b7uga73n5s.apps.googleusercontent.com", // Web client ID
  ): GoogleSignIn();
  ...
  Future<GoogleSignInAuthentication> googleLogin() async {
    ...
    // Assure-toi d'initialiser GoogleSignIn avec le clientId Web
    googleAccount = await _googleSignIn.signIn();
    ...
    // Crée un credential Firebase avec les tokens Google
    final credential = GoogleAuthProvider.credential(
      accessToken: auth.accessToken,
      idToken: auth.idToken,
    );

    // Connecte-toi avec Firebase Auth
  ...
  Future  socialLogin(SocialLoginModel socialLogin, Function callback) async {
    ...
    if (apiResponse.response?.statusCode == 200 && apiResponse.response != null) {
      print("-----------------------(SOCIAL LOGIN API 2 )-----------${apiResponse.response?.data}");
      Map map = apiResponse.response?.data;
      String? message = '';
      String? token = '';
      String? tempToken = '';
      ...
      if(token != null){
        print("🧪 Forcing callback call for debug");
        await updateAuthToken(token);
        final ProfileProvider profileProvider = Provider.of<ProfileProvider>(Get.context!, listen: false);
